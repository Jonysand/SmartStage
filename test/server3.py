...
def receive_data(sock, addr):
    global tone_flag, rec_flag, record, interval, start_time, tone_name_list, ten_sec_flag, IP_list
    old_note = [0,0,0,0,0,0,0]
    data_size = 3040
    while True:
        data = sock.recv(1024)
        if "conn" in data.decode():
            this_inst_name = data.decode()[4:]
            noti_connected.play(0)
            print (this_inst_name+" Connected !")
            ip = addr[0]
            this_inst = [this_inst_name, ip]
            IP_list.append(this_inst)
            continue
        if not data:
            noti_disconnected.play(0)
            print (this_inst_name+" disconnected")
            IP_list.remove(this_inst)
            break
        ...
        while received_size<data_size:
            ...
            received_size += len(data)
            received_data += data
        out_mes = received_data.decode()
        out_list = json.loads(out_mes)
...
tone_list = add_new_sound(tone_list, "guitar_note")
...
global IP_list
IP_list = [] # to save IP addrs from instruments
